...
use App\Models\GroupActivityTracking;
use Encore\Admin\Controllers\AdminController;
use Encore\Admin\Form;
use Encore\Admin\Grid;
use Encore\Admin\Show;

class GroupActivityTrackingController extends AdminController
{
    /**
     ...
     * @var string
     */
    protected $title = 'Group Activity Tracking';
    ...
    {
        $grid = new Grid(new GroupActivityTracking());
        ...
        $grid->column('check_in', __('Check in'))->display(function($types){
            if ($types == 1) {
                return "<span class='label label-success'>9:30 AM</span>";
            } 
        });
        $grid->column('check_out', __('Check out'))->display(function($types){
            if ($types == 0) {
                return "<span class='label label-info'>6:30 PM</span>";
            } 
        });
    ...
    {
        $show = new Show(GroupActivityTracking::findOrFail($id));
    ...
    {
        $form = new Form(new GroupActivityTracking());
        ...
        $form->select('check_in', __('Check in'))->options([1 => '9:30 AM'])->rules(function ($form) {
            return 'required';
        });
        $form->select('check_out', __('Check out'))->options([ 0 => '6:30 PM'])->rules(function ($form) {
            return 'required';
        });
